class Api::V1::CampusController < ApplicationController
  ...
  def index
	...
	return invalid_institution unless  @institution.present?
  	...
  	if resource.is_admin?
  	  ...
  	  return no_campus if @campus.empty?
  	else
  	  @campus =  Campu.where(:institution_id => params[:institution_id])
  	  return no_campus if @campus.empty?
  	end
  ...
  def no_campus
  	render json: { success: false, msg: "No campuses in this institution"}, status: :unprocessable_entity
  end
  ...
  def set_csrf_header
    ...
    # def set_institution
    #   @institution = Campu.includes(:topics).find(params[:id]) #include(:topics) includes all teh topics in that institution pre-emptively, saving alot ot memory and computing resources
    # end
