def table
	...
	nodes.each_with_index do |par, index| #Go throug all paragraph tags and process each paragraph one at at time
		...
		if regex.match(par.text.squish) #If the current paragraph's content is QUESTION x we have found a question, read everything till you find QUESTION X
			...
			# temp << par.text + space
			next_par = nodes[index+next_paragraph_counter]
				...
				if regex.match(next_par_in_qn.text.squish)
					...
					# temp << next_par.to_html
						...
						if next_par_in_qn.nil? || regex.match(next_par_in_qn.text.squish )
							...
							break if next_par_in_qn.text.squish  == "END" || next_paragraph_counter ==  nodes.count || regex.match(next_par_in_qn.text.squish )
							next_paragraph_counter +=1
							next_index = index+next_paragraph_counter
							p " NEXT INDEX #{next_index}"
							next_par_in_qn = nodes[index+next_paragraph_counter]

	

							p"next paragraph is WWWWWW  #{next_par_in_qn.text.squish}  || #{index} || #{next_paragraph_counter}  "
